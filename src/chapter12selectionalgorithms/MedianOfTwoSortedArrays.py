...
def findKthSmallest(A, B, k):
    ...
    # The median must be in the right part of B or left part of A
        ...
        return findKthSmallest(A, B[stepsB + 1 :], k - stepsB - 1)
    # The median must be in the right part of A or left part of B
    else:
        return findKthSmallest(A[stepsA + 1 :], B, k - stepsA - 1)
...
def findMedianSortedArrays(A, B):
    ...
    if (len(A) + len(B)) % 2 == 1:
        ...
        # middle two elements.
        return (
            findKthSmallest(A, B, (len(A) + len(B)) / 2 + 1)
            + findKthSmallest(A, B, (len(A) + len(B)) / 2)
        ) / 2.0
